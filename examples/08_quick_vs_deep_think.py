"""
Same query, two reasoning modes — speed vs depth.

`quick` returns in a couple of seconds with single-pass retrieval + answer
synthesis. `deep_think` runs an agent loop that decomposes the question,
retrieves multiple times, and reasons over intermediate results — slower, but
...
for mode in ("quick", "deep_think"):
    t0 = time.perf_counter()
    r = client.search.intelligent(project_id=louisville.id, query=query, mode=mode)
    print(f"[{mode:10s}  {(time.perf_counter() - t0):>5.1f}s]  {r.answer[:200]}...\n")
...
# [deep_think  18.7s]  Across both years, the council shifted from a single-line-item allocation
